...
def get_results(pars, z, ombh2, omch2, cosmomc_theta, tau, omk, As, ns):
...
df_in = pd.read_pickle(in_file)[:10]
print("There are {} total instances to run.".format(len(df_in)))
# output chain
df_out = df_in.copy()
df_out['cf_peak'] = 0
df_out['cf_dip'] = 0
...
param_names = ['omegabh2', 'omegach2', 'theta', 'tau', 'omegak', 'A*', 'ns']

ctr = 0
for i in df_out.index:
    ombh2, omch2, cosmomc_theta, tau, omk, As, ns = df_out.loc[i, param_names]
    cosmomc_theta /= 100
    As *= 1e-9
    results = get_results(pars, 0.0,
                          ombh2, omch2, cosmomc_theta, tau, omk, As, ns)
    try:
    ...
    if ctr % 100 == 0:
        print("{} out of {} lines done, {} to go.".format(
            ctr+1, len(df_in), len(df_in) - ctr))
    ctr += 1
